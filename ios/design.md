...
Family uses Go's existing Newsreader headline font, ivory and warm-white surfaces, forest-green accents, caregiver colors, line icons, and rounded bottom sheets. The masthead has a settings button. The workload chart is read-only and **stays put on all four panels**. *(It used to yield its space to Locations and Templates. That was wrong: the panel below then jumped up and down as you moved along the segments, which reads as the screen re-laying itself out rather than as a tab swapping. A segmented control's whole promise is that only the thing below it changes.)* Swap Next and rebalance actions are removed, including Plan's rebalance entry points.

Family's body copy runs at 12.5–14.5px rather than the 10–12px it launched with. This is a setup screen read at rest, not a countdown read at a stoplight; the compression that makes Go's rail scannable only made addresses, relationships and template details hard to read here.
...
**Templates are grouped by what they are, and each row carries only three things.** The heading is the category — Sports, Arts & music, Family & social, School, Health — with a colour dot matching the activity mix chart and a count on the right. Under it, each template is one row: its name, then its start time and the children it is for. Location, the caregiver, the activity type and the child tags are all in the editor, one tap away, because a list of shortcuts is something you scan for the right name rather than read. A template declares its own `category`; an event without one is read from its title, with Sports tested before Health so a gymnastics clinic is not a doctor's appointment, and the social rule above School so a birthday drop-off is not a school run.

The editor puts name, time, place, and multiple-child selection first; caregiver and activity type sit in a disclosure. Plan reduces each template to a title, start time, and child names. Full details open only when the user chooses a shortcut.
...
### Kids Stats is drawn, not written

The panel answers two questions a list of event titles cannot: **where is the time going, and who is doing the driving.** A row of chips scopes the headline figures to one child or to everyone; the per-child care split always shows every child, because its whole point is the comparison.

Four counters sit at the top — scheduled time, journeys, the busiest day, and the activity the week has most of. Below them, **the activity mix** is one stacked bar over a legend: Sports `#7fa375`, Arts `#bda170`, Social `#c2ba6b`, School `#7f9fad`, Health `#c98b6b`. Below that, **time with each caregiver** gives every child one bar segmented by who is with them, in the same chart-strength caregiver hues the workload card and Plan already use — so a colour means the same person everywhere in the app. One legend serves all the children's bars rather than repeating under each.

Bars carry a visually hidden label per segment, so a screen reader gets "Mom, 13.3 hours" rather than an unlabelled rectangle. The legend row is a three-column grid — dot, name, figure — so a name that wraps cannot drag the figure out of its column.

The figures are **scheduled activity time**, not driving time; driving minutes stay on the workload card above and on the roster rows, and the panel's footnote says so. An earlier pass rendered all of this as sentences under each child's name, which made the reader do the arithmetic the chart is for.

### The roster names a person once, and says what they carry

Each caregiver is one card: their identity disc at 42px, their name in the headline serif, their relationship and starting base, and then the week itself — handoffs and driving minutes, in forest. Shared planning rules follow as a definition list (buffer, traffic adjustment, dinner protection) with a single action into Settings, rather than as a sentence with the values buried in it.
