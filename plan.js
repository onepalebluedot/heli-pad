...
(() => {
  ...
  function sheetContent() {
    ...
    if(kind==='reviewed')return {title:'Week reviewed',body:`<div class="pn-empty"><div class="pn-success">${icon('check')}</div><strong>A plan you can come back to.</strong>${data.remaining?`${data.remaining} flagged events remain visible. Review saved does not mean every event is ready.`:'All events have a caregiver and no detected planning risks.'}</div>`,foot:btn('Back to the week','close')};
