...
(() => {
  ...
  const dayTags=events=>events.map(e=>`<span>${shortDay(e.date)}</span>`).join('');
  ...
  function decisionCard(e) {
    return `<article class="pn-decision-card ${C.unassigned(e)?'urgent':''}"><span class="pn-decision-icon">${icon(C.unassigned(e)?'alert':'flag')}</span><div><small>${shortDay(e.date)} · ${clock(e.time)}</small><h3>${esc(e.title)}</h3><p>${esc(kidsOf(e).join(', '))} · ${esc(e.location)}</p></div>${btn(C.unassigned(e)?'Assign':'Review','assign','pn-card-action',`data-id="${esc(e.id)}"`)}</article>`;
  }
  ...
  function reviewSheet() {
    ...
    return {title:'A few things to line up',eyebrow:range(view.week),body:`<p class="pn-muted">${stats.ready} of ${stats.total} ready. Ready means assigned, confirmed, and no detected timing or route risks.</p><div class="pn-pills">${[['all',`All · ${issues.length}`],['driver',`Unassigned · ${stats.missing}`],['overlap','Overlaps'],['tentative','Tentative']].map(([id,label])=>btn(label,'review-filter','pn-pill',`data-value="${id}" aria-pressed="${view.reviewFilter===id}"`)).join('')}</div>${items.length?items.map(e=>`<article class="pn-review-item"><small>${shortDay(e.date)} · ${window.isAllDay && window.isAllDay(e) ? 'All day' : clock(e.time)} · ${esc(e.owner)}</small><h3>${esc(e.title)}</h3><p>${e.risks.map(r=>esc(r.label)).join(' · ')}</p>${btn('Review caregiver ↗','assign','pn-text',`data-id="${esc(e.id)}"`)} ${btn('Edit event','event','pn-text',`data-id="${esc(e.id)}"`)}</article>`).join(''):`<div class="pn-empty">${icon('check')}<strong>Nothing flagged here.</strong>Try another filter or finish your review.</div>`}${btn(`${state.buffer} min travel buffer`,'rules','pn-text')}<p class="pn-muted">Suggestions use sample routes and schedule rules. No AI service or live traffic is connected.</p>`,foot:btn(issues.length?'Save review · keep flags':'Mark week reviewed','review-save')};
  }
  ...
  function eventSheet(data) {
    ...
    return {title:e?'Edit this handoff':'Plan a handoff',eyebrow:e?.seriesId?'Editing this occurrence only':'Add to the week',body:`<form id="pnEventForm" class="pn-event-form"><section><div class="go-sec-head"><label>What is happening?</label><span>Keep it scannable</span></div><input class="go-field" aria-label="Event title" name="title" value="${esc(d.title)}" maxlength="100" placeholder="Soccer, school pickup, family dinner…" required></section>
      ...
      ${!e?`<section class="go-panel pn-repeat-panel"><div class="go-sec-head"><label>${icon('repeat')} Repeat this task</label><span>Set the rhythm once</span></div><div class="go-two"><label><span class="go-mini">Repeat</span><select class="go-field" name="repeat"><option value="none" ${d.repeat!=='weekly'?'selected':''}>Does not repeat</option><option value="weekly" ${d.repeat==='weekly'?'selected':''}>Every week</option></select></label><label><span class="go-mini">Occurrences</span><input class="go-field" type="number" min="1" max="52" name="count" value="${d.count||4}"></label></div><p class="pn-muted">Each occurrence stays editable after the routine is created.</p></section>`:''}
  ...
  function handle(action,button) {
    ...
    if(action==='routine'){openSheet('routine',{key:value});return;}
